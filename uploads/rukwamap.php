...
<body>
	...
	<input id="slide" type="range" min="0" max="1" step="0.1" value="1" onchange="updateOpacity(this.value)">
				<script src='data/exp_RukwaWaterpoints.js' ></script>
				
				<script src='data/exp_RukwaRegion.js' ></script>
				
	<script>
		var map = L.map('map', { zoomControl:true }).fitBounds([[-9.07751839913,28.4371023291],[-5.41522725787,34.3625346509]]);
	...
	var layerOrder=new Array();
				...
				feature_group.addLayer(exp_RukwaWaterpointsJSON);
				layerOrder[layerOrder.length] = exp_RukwaWaterpointsJSON;
				for (index = 0; index < layerOrder.length; index++) {
					feature_group.removeLayer(layerOrder[index]);feature_group.addLayer(layerOrder[index]);
				}
				
						//add comment sign to hide this layer on the map in the initial view.
						exp_RukwaWaterpointsJSON.addTo(map);
	    ...
	    title.update = function () {
			this._div.innerHTML = '<h2>Rukwa Region</h2>WSS Master Plan'
		};
		title.addTo(map);
	...
	L.control.layers({'OSM Standard': basemap},{"RukwaWaterpoints": exp_RukwaWaterpointsJSON,"RukwaRegion": exp_RukwaRegionJSON},{collapsed:false}).addTo(map);
